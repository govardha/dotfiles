...
- Use these inline patterns for fatal exits — do not use bare `exit`:
  - operational failure:   `log_critical "msg"; exit 5`
  - bad environment/config: `log_critical "msg"; exit 3`
  - bad arguments:          `log_error "msg"; exit 2`
  - missing dependency:     `log_critical "msg"; exit 4`
...
- **`sendmsg.py`** — use when you need to target a specific channel of your own
  choosing: `/apps/ops/sysbot/sendmsg.py -c "#my-channel" "message"`
...
[[ -z "${MIAX_ENV}" ]] && { log_critical "MIAX_ENV is not set"; exit 3; }
